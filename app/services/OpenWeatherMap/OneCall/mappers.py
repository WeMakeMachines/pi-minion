...
class Mapper:
    ...
    def __map_hour(self, hour: Hour):
        ...
        return {
            ...
            "description": Forecast(
                title=hour["weather"][0]["main"], body=hour["weather"][0]["description"]
            ),
            "clouds": Clouds(
                cover=hour["clouds"], precipitation_chance=precipitation_chance
            ),
    ...
    def __map_day(self, day: Day):
        return {
            "sun": Sun(
                sunrise=day["sunrise"], sunset=day["sunset"], uv_index=day["uvi"]
            ),
            "description": Forecast(
                title=day["weather"][0]["main"], body=day["weather"][0]["description"]
            ),
            "clouds": Clouds(cover=day["clouds"], precipitation_chance=day["pop"]),
            "temperature": {
    ...
    def map_now(self, now: Hour):
        ...
        mapped_now.update(
            {
                "sun": Sun(
                    sunrise=now["sunrise"], sunset=now["sunset"], uv_index=now["uvi"]
                ),
            }
        )
